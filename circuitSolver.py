...
#Coded by: Vijay Jayaram, vijayj, Section H
#Mentor: Dylan Swen
...
class Circuit(object):
	...
	def addConnect(self, connect):
		src = connect.src
		dest = connect.dest
		#For each new src, make a node out of it
		if src not in self.nodes:
			self.nodes[src] = Node(src)
		#Same for new dest
		if dest not in self.nodes:
			self.nodes[dest] = Node(dest)
		#That node adds the connection as well
		self.nodes[src].addConnect(connect)
		self.nodes[dest].addConnect(connect.inverse())
	
	#adds ground to the circuit at specified location
	def addGround(self,location):
		self.ground = location
		
	#For testing purposes, prints the attributes of all circuit elements
	def printCircuit(self):
		for loc in self.nodes:
			for connect in self.nodes[loc].connections:
				print connect.src, connect.dest, connect.elem, connect.value
		print self.ground
	
	####################################################
	## FUNCTION THAT ACTUALLY SOLVES THE CIRCUIT!
	####################################################
	...
	def createEquations(self):
		...
		for nodeLocation in self.nodes:
			...
			for connect in self.nodes[nodeLocation].connections:
				nodeLoc1 = voltLocations[connect.src]
				nodeLoc2 = voltLocations[connect.dest]
				#testing situation for two nodes separated by a voltage source
				#the equation is n1 - n2 = voltageValue
				if connect.elem == "voltage":
					equation = [0]*(dim+1)
					equation[nodeLoc1] = 1
					equation[nodeLoc2] = -1
					equation[dim] = -connect.value
			...
			#matrix.
			for connect in self.nodes[nodeLocation].connections:
				if connect.elem == "resistor":
					canKCL = True
			if canKCL:
				self.KCLEquation = [0] * (self.dim + 1)
				self.visitedConnects = set()
				self.recursivelyFindCurrent(nodeLocation)
				voltages.append(self.KCLEquation)	
		return voltages			
	
	#This function uses the node analysis algorithm to generate KCL equations
	#It goes through each of the connections for a node, and generates a KCL
	#That is, the sum of all entering currents equals zero.
	#So, for each connection with a resistor, current is found by dividing 
	...
	def recursivelyFindCurrent(self,location):
		voltLocations = self.voltLocations
		for connect in self.nodes[location].connections:
			#if the connection was already visited, don't do anything
			#otherwise, this would recurse infinitely
			if connect in self.visitedConnects:
				pass
			else:
				self.visitedConnects.add(connect)
				#Base case: two nodes are connected by a resistor
				if connect.elem == 'resistor':
					srcIndex = self.voltLocations[connect.src]
					destIndex = self.voltLocations[connect.dest]
					#currents going into, so it's src - dest
					self.KCLEquation[destIndex] += -1.0/connect.value
					self.KCLEquation[srcIndex] += 1.0/connect.value
				#Recursive case: nodes connected by a non-resistive element
				#Go through using KCLs and add them up to get the current
				else:
					self.recursivelyFindCurrent(connect.dest)
...
#Includes locations of both nodes and the
#elements that connect them.
class Connection(object):
	def __init__(self,elem,value,src,dest):
		assert(elem == "voltage" or elem == "resistor")
		self.src = src
		self.dest = dest
		self.elem = elem
		self.value = value
	...
	def printInfo(self):
		print self.src,self.dest,self.elem,self.value
	
	#Switching source and destination
	def inverse(self):
		#Backwards voltage is negative
		if self.elem == "voltage":
			return Connection(self.elem,-self.value,self.dest,self.src)
		return Connection(self.elem,self.value,self.dest,self.src)

#Class for each node, or intersection of gridlines, on the circuit
class Node(object):
	#initializes voltage/current to 0 and introduces a dictionary
	#that holds all elements and the node to which they connect
	...
	def addConnect(self,connect):
		assert(self.location == connect.src)
	...
	if inBounds(event.x,event.y):
		...
		if canvas.data.isClicked:
			...
			if lastWire[0]!=lastWire[1]:
				connect = Connection("voltage",0,lastWire[0],lastWire[1])
				canvas.data.circuit.addConnect(connect)
...
def addVoltageToGrid(x,y):
	...
	if voltValue != None:
		...
		a = Connection("voltage",voltValue,(x,y+40),(x,y-40))
		canvas.data.circuit.addConnect(a)
...
def addResistorToGrid(x,y):
	...
	resValue = 0
	while resValue <= 0:
		resValue = tkSimpleDialog.askfloat(rPrompt[0],rPrompt[1])
	...
	if resValue != None:
		...
		b = Connection("resistor",resValue,(x-40,y),(x+40,y))
		canvas.data.circuit.addConnect(b)
...
def keyPressed(event):
	...
	if event.keysym == "p":
		canvas.data.circuit.printCircuit()
	#shows element pane
	if event.keysym =="e": 
...
def drawGridLines():
	...
	boxSize = 20
	color = "Dark Green"
	#Drawing vertical and horizontal lines, spaced by 20 pixels each
	for i in xrange(dimension/boxSize):
		canvas.create_line(i*boxSize,0,i*boxSize,dimension,fill = color)
		canvas.create_line(0,i*boxSize,dimension,i*boxSize,fill = color)
